<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
  <meta charset="UTF-8">
  <title>🛒 متجر PIZZA HOT – جديد</title>
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700;900&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="store.css">
</head>
<body>
<header>
  <a href="index.html">🏠 الرئيسية</a>
  <a href="promo_offer.html">🔥 العرض</a>
  <a href="store.v2.html">🛒 المتجر الجديد</a>
</header>
<div class="overlay">
  <h1>🛒 متجر PIZZA HOT – نسخة محسّنة</h1>

  <div class="note-box">
    🎟️ الكود اليومي: <span id="daily-coupon-box">—</span><br>
    يمكنك نسخه ووضعه في الحقل الرئيسي أو الثانوي للحصول على خصم كامل أو جزئي.
  </div>

  <div id="auto-discount-alert" style="display:none">
    🎁 تم تفعيل خصم تلقائي اليوم — لا حاجة لإدخال كود في الحقل الرئيسي.
  </div>
<div class="section">
  <h2>🍕 قائمة البيتزا</h2>
  <div class="table-wrapper">
    <table id="pizza-menu">
      <thead>
        <tr>
          <th>النوع</th>
          <th>الحجم</th>
          <th>السعر</th>
          <th>الكمية</th>
          <th>الإجمالي</th>
          <th>أضف</th>
        </tr>
      </thead>
      <tbody>
        <tr data-item="بيتزا دجاج">
          <td>بيتزا دجاج</td>
          <td>
            <select class="size">
              <option value="65">XL – 65₪</option>
              <option value="45">L – 45₪</option>
              <option value="33">M – 33₪</option>
              <option value="17">S – 17₪</option>
            </select>
          </td>
          <td><span class="price">65₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">65₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>

        <tr data-item="بيتزا خضار">
          <td>بيتزا خضار</td>
          <td>
            <select class="size">
              <option value="50">XL – 50₪</option>
              <option value="38">L – 38₪</option>
              <option value="20">M – 20₪</option>
              <option value="14">S – 14₪</option>
            </select>
          </td>
          <td><span class="price">50₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">50₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>

        <tr data-item="بيتزا سلامي">
          <td>بيتزا سلامي</td>
          <td>
            <select class="size">
              <option value="55">XL – 55₪</option>
              <option value="40">L – 40₪</option>
              <option value="23">M – 23₪</option>
              <option value="15">S – 15₪</option>
            </select>
          </td>
          <td><span class="price">55₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">55₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>

        <tr data-item="بيتزا نقانق">
          <td>بيتزا نقانق</td>
          <td>
            <select class="size">
              <option value="55">XL – 55₪</option>
              <option value="40">L – 40₪</option>
              <option value="23">M – 23₪</option>
              <option value="15">S – 15₪</option>
            </select>
          </td>
          <td><span class="price">55₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">55₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>

        <tr data-item="بيتزا تونة">
          <td>بيتزا تونة</td>
          <td>
            <select class="size">
              <option value="55">XL – 55₪</option>
              <option value="40">L – 40₪</option>
              <option value="23">M – 23₪</option>
              <option value="15">S – 15₪</option>
            </select>
          </td>
          <td><span class="price">55₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">55₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>

        <tr data-item="بيتزا مارجريتا">
          <td>بيتزا مارجريتا</td>
          <td>
            <select class="size">
              <option value="50">XL – 50₪</option>
              <option value="38">L – 38₪</option>
              <option value="20">M – 20₪</option>
              <option value="14">S – 14₪</option>
            </select>
          </td>
          <td><span class="price">50₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">50₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>

        <tr data-item="كلزوني">
          <td>كلزوني</td>
          <td>
            <select class="size">
              <option value="60">XL – 60₪</option>
              <option value="40">L – 40₪</option>
              <option value="32">M – 32₪</option>
              <option value="16">S – 16₪</option>
            </select>
          </td>
          <td><span class="price">60₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">60₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
      </tbody>
    </table>
  </div>
</div>
<div class="section">
  <h2>🍟 أطباق جانبية</h2>
  <div class="table-wrapper">
    <table id="sides-menu">
      <thead>
        <tr>
          <th>الصنف</th>
          <th>السعر</th>
          <th>الكمية</th>
          <th>الإجمالي</th>
          <th>أضف</th>
        </tr>
      </thead>
      <tbody>
        <tr data-item="بطاطا صغير" data-price="5">
          <td>بطاطا صغير</td>
          <td><span class="price">5₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">5₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
        <tr data-item="بطاطا كبير" data-price="10">
          <td>بطاطا كبير</td>
          <td><span class="price">10₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">10₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
        <tr data-item="زنجر صغير" data-price="12">
          <td>زنجر صغير</td>
          <td><span class="price">12₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">12₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
        <tr data-item="زنجر كبير" data-price="25">
          <td>زنجر كبير</td>
          <td><span class="price">25₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">25₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
        <tr data-item="أجنحة جنان صغير" data-price="12">
          <td>أجنحة جنان صغير</td>
          <td><span class="price">12₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">12₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
        <tr data-item="أجنحة دجاج كبير" data-price="24">
          <td>أجنحة دجاج كبير</td>
          <td><span class="price">24₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">24₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
      </tbody>
    </table>
  </div>
</div>
<div class="section">
  <h2>🥤 مشروبات</h2>
  <div class="table-wrapper">
    <table id="drinks-menu">
      <thead>
        <tr>
          <th>المشروب</th>
          <th>السعر</th>
          <th>الكمية</th>
          <th>الإجمالي</th>
          <th>أضف</th>
        </tr>
      </thead>
      <tbody>
        <tr data-item="كولا 1.25 لتر" data-price="4">
          <td>كولا 1.25 لتر</td>
          <td><span class="price">4₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">4₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
        <tr data-item="كولا 2 لتر" data-price="6">
          <td>كولا 2 لتر</td>
          <td><span class="price">6₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">6₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
        <tr data-item="عصير مانجو" data-price="5">
          <td>عصير مانجو</td>
          <td><span class="price">5₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">5₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
        <tr data-item="مياه معدنية" data-price="3">
          <td>مياه معدنية</td>
          <td><span class="price">3₪</span></td>
          <td><input class="qty" type="number" min="1" value="1"></td>
          <td class="total-cell">3₪</td>
          <td><button class="add-btn">أضف</button></td>
        </tr>
      </tbody>
    </table>
  </div>
</div>
<div id="cart-manager">
  <input id="user-name" placeholder="👤 اسمك (اختياري)" />
  <input id="user-address" placeholder="📍 العنوان/هاتفك (اختياري)" />
  <input id="user-coupon" placeholder="🎟️ كود الخصم الرئيسي (يُحسب كامل)" />
  <input id="secondary-coupon" placeholder="🎟️ كود إضافي (يُحسب ربع الخصم)" />
  <button id="start-btn">☑ معاينة الطلب</button>
  <button id="clear-cart">🗑️ تفريغ السلة</button>
  <button id="send-wa">📤 إرسال الطلب</button>
  <div id="cart-preview"></div>
</div>
</div>
<div class="footer">
  📍 بلعين – شارع صفا – رام الله<br>
  © 2025 PIZZA HOT — جميع الحقوق محفوظة.
</div>

<script src="cart.js"></script>
<script src="discountEngine.js"></script>
<script src="discountRules.js"></script>
<script src="messageBuilder.js"></script>
<script src="dailyCoupon.js"></script>
<script src="store.js"></script>

</body>
</html>
